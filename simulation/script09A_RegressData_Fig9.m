...
nr_boot=size(data{2}.allboots,3); 
% loop regression over electrodes and bootstraps
v_area=zeros(length(data),1);
reg_out(1).stats=zeros(length(data),nr_boot,4); 
reg_out(2).stats=zeros(length(data),nr_boot,4); 
reg_out(3).stats=zeros(length(data),nr_boot,5); 
reg_out(4).stats=zeros(length(data),nr_boot,4); 
...
for k=1:length(data)
    ...
    for bs=1:nr_boot
        ...
        fmri_d=data{k}.allbootsS12(:,bs);
        ecog_bb=data{k}.bb_even(:,bs);
        ecog_g=data{k}.gamma_even(:,bs);
        ecog_a=data{k}.alpha_even(:,bs);
        ...
        for m=1:length(ecog_in)
            ...
            if ~isnan(stats1.rsquare) % nans if alpha all zeros
                reg_out(m).stats(k,bs,1)=stats1.rsquare;
                reg_out(m).stats(k,bs,2)=stats1.adjrsquare;
                reg_out(m).stats(k,bs,3:2+length(stats1.beta))=stats1.beta; % 1 is the intercept
            else
                reg_out(m).stats(k,bs,1) = 0;
                reg_out(m).stats(k,bs,2) = 0;
                reg_out(m).stats(k,bs,3:2+length(stats1.beta)) = 0;
            end
            clear stats1
        end
        clear ecog_in
    ...
    for bs=1:nr_boot
        ...
        ecog_bb=data{k}.bb_odd(:,bs);
        ecog_g=data{k}.gamma_odd(:,bs);
        ecog_a=data{k}.alpha_odd(:,bs);
        ...
        for m=1:length(ecog_in)
            ...
            r2_crossval_out(k,m,bs)=corr(pred_fmri,fmri_d).^2;
        end
        clear ecog_in
    end        
end
